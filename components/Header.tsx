...
const Header: React.FC = () => {
    ...
    const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
        `block py-2 pr-4 pl-3 uppercase tracking-wider transition-colors duration-300 ${isActive ? 'text-red-500' : 'text-gray-300 hover:text-white'}`;
    
    return (
        <header className="bg-black bg-opacity-80 backdrop-blur-sm sticky top-0 z-50 border-b border-gray-800">
            <div className="container mx-auto flex justify-between items-center p-4">
                <Link to="/home" className="flex items-center space-x-2">
                    <MotoIcon />
                    <span className="text-2xl font-bold tracking-tighter text-white">MotoManiacs</span>
                </Link>
                ...
                <div className="hidden md:flex items-center space-x-8">
                    ...
                    {currentUser && <Button onClick={handleLogout} size="sm" variant="secondary">Logout</Button>}
                </div>
            ...
            {isOpen && (
                <div className="md:hidden bg-gray-900 p-2">
                    ...
                    {currentUser && <div className="p-2"><Button onClick={handleLogout} size="sm" variant="secondary" className="w-full">Logout</Button></div>}
                </div>
            )}
        </header>
    );
