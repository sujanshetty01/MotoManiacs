...
export const subscribeToUserBookings = (
  ...
  const setupListener = (useOrder: boolean) => {
    const queryToUse = useOrder ? qWithOrder : qWithoutOrder;
    ...
    return onSnapshot(
      ...
      (querySnapshot) => {
        ...
        // If using unordered query, sort in memory
        if (!useOrder) {
          bookings.sort((a, b) => {
      ...
      (error: any) => {
        // If ordered query fails due to missing index, fall back to unordered
        if (error.code === 'failed-precondition' && useOrderedQuery) {
          console.warn('Composite index missing for user bookings. Falling back to unordered query.');
          console.warn('For better performance, create the index:', error.message);
          if (error.message && error.message.includes('https://')) {
            const indexLink = error.message.match(/https:\/\/[^\s]+/)?.[0];
            if (indexLink) {
              console.warn('Index creation link:', indexLink);
            }
          }
          
          // Switch to unordered query
