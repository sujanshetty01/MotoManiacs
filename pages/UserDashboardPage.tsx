            ...
            {userBookings.length > 0 ? (
                <div className="space-y-6">
                    {userBookings.map(({ id, event, tickets, totalPrice, status, bookingDate }) => (
                        <div key={id} className="bg-gray-900 border border-gray-800 rounded-lg p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            {event ? (
                                <>
                                    <div className="flex-grow">
                                        <h2 className="text-2xl font-bold text-white">{event.title}</h2>
                                        <p className="text-gray-400">{new Date(event.date).toLocaleDateString()}</p>
                                        <p className="text-sm text-gray-500 mt-1">Booked on: {new Date(bookingDate).toLocaleDateString()}</p>
                                    </div>
                                    ...
                                    <div className="flex flex-col space-y-2 mt-4 md:mt-0 md:ml-6">
                                        ...
                                        {status === 'Confirmed' && (
                                            <button onClick={() => cancelBooking(id)} className="bg-gray-700 text-white font-bold py-2 px-4 rounded-md hover:bg-gray-600 transition-colors duration-300 text-sm">Cancel</button>
                                        )}
                                    </div>
                                </>
                            ) : (
