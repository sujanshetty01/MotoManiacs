...
const EventsPage: React.FC = () => {
  ...
  const FilterButton = ({ type, label }: { type: EventType; label: string }) => (
    <button
      ...
      className={`px-4 py-2 rounded-md font-semibold transition-colors duration-300 ${filter === type ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
    >
      {label}
    </button>
  );
  ...
  return (
    <div className="container mx-auto px-6 py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-400 mt-2">Find your next adrenaline rush.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="relative w-full md:w-auto">
          <input
            ...
            className="bg-gray-800 border-2 border-gray-700 text-white rounded-md py-2 px-4 w-full md:w-64 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
          />
        </div>
      </div>
